    ...
    case BINDER_TYPE_FD: {
      if (obj.cookie != 0) {
        if (outAshmemSize != NULL) {
          // If we own an ashmem fd, keep track of how much memory it
          // refers to.
...
void release_object(const sp<ProcessState> &proc, const flat_binder_object &obj,
                    const void *who) {
...
status_t flatten_binder(const sp<ProcessState> & /*proc*/,
  ...
  if (binder != NULL) {
    ...
    if (!local) {
      ...
      obj.binder =
          0; /* Don't pass uninitialized stack data to a remote process */
      obj.handle = handle;
      obj.cookie = 0;
    } else {
  ...
  if (binder != NULL) {
    ...
    if (real != NULL) {
      ...
      if (!local) {
        ...
        obj.binder = 0; /* Don't pass uninitialized stack data to a
                           remote process */
        obj.handle = handle;
        obj.cookie = 0;
      } else {
    ...
    switch (flat->type) {
      ...
      case BINDER_TYPE_HANDLE:
        *out = proc->getStrongProxyForHandle(flat->handle);
        return finish_unflatten_binder(static_cast<BpBinder *>(out->get()),
                                       *flat, in);
    }
  }
  return BAD_TYPE;
...
const uint8_t *Parcel::data() const { return mData; }
...
size_t Parcel::dataPosition() const { return mDataPos; }

size_t Parcel::dataCapacity() const { return mDataCapacity; }
...
bool Parcel::allowFds() const { return mAllowFds; }
...
bool Parcel::enforceInterface(const String16 &interface,
  ...
  if ((threadState->getLastTransactionBinderFlags() & IBinder::FLAG_ONEWAY) !=
      0) {
...
const binder_size_t *Parcel::objects() const { return mObjects; }

size_t Parcel::objectsCount() const { return mObjectsSize; }
...
void *Parcel::writeInplace(size_t len) {
  ...
  restart_write:
    ...
    if (padded != len) {
      ...
      static const uint32_t mask[4] = {0x00000000, 0xffffff00, 0xffff0000,
                                       0xff000000};
#endif
#if BYTE_ORDER == LITTLE_ENDIAN
      static const uint32_t mask[4] = {0x00000000, 0x00ffffff, 0x0000ffff,
                                       0x000000ff};
#endif
...
status_t Parcel::writeUint32(uint32_t val) { return writeAligned(val); }
...
status_t Parcel::writeInt64(int64_t val) { return writeAligned(val); }

status_t Parcel::writeUint64(uint64_t val) { return writeAligned(val); }

status_t Parcel::writePointer(uintptr_t val) {
  return writeAligned<binder_uintptr_t>(val);
}

status_t Parcel::writeFloat(float val) { return writeAligned(val); }
...
status_t Parcel::writeDouble(double val) { return writeAligned(val); }
...
bool Parcel::readBool() const { return readInt32() != 0; }

status_t Parcel::writeBool(bool val) { return writeInt32(int32_t(val)); }
...
status_t Parcel::writeNativeHandle(const native_handle *handle) {
  if (!handle || handle->version != sizeof(native_handle)) return BAD_TYPE;
  ...
  if (result < 0) {
    ...
    if (ptr == MAP_FAILED) {
      ...
      if (result < 0) {
        ...
        status = writeInt32(mutableCopy ? BLOB_ASHMEM_MUTABLE
...
status_t Parcel::writeNoException() { return writeInt32(0); }
...
status_t Parcel::read(void *outData, size_t len) const {
  ...
  if ((mDataPos + pad_size(len)) >= mDataPos &&
      (mDataPos + pad_size(len)) <= mDataSize && len <= pad_size(len)) {
...
status_t Parcel::readInt32(int32_t *pArg) const { return readAligned(pArg); }
...
uint32_t Parcel::readUint32() const { return readAligned<uint32_t>(); }

status_t Parcel::readInt64(int64_t *pArg) const { return readAligned(pArg); }

int64_t Parcel::readInt64() const { return readAligned<int64_t>(); }

status_t Parcel::readUint64(uint64_t *pArg) const { return readAligned(pArg); }

uint64_t Parcel::readUint64() const { return readAligned<uint64_t>(); }
...
status_t Parcel::readFloat(float *pArg) const { return readAligned(pArg); }

float Parcel::readFloat() const { return readAligned<float>(); }
...
status_t Parcel::readDouble(double *pArg) const { return readAligned(pArg); }

double Parcel::readDouble() const { return readAligned<double>(); }

#endif

status_t Parcel::readIntPtr(intptr_t *pArg) const { return readAligned(pArg); }

intptr_t Parcel::readIntPtr() const { return readAligned<intptr_t>(); }
...
const char16_t *Parcel::readString16Inplace(size_t *outLen) const {
  ...
  if (size >= 0 && size < INT32_MAX) {
    *outLen = size;
    const char16_t *str =
        (const char16_t *)readInplace((size + 1) * sizeof(char16_t));
    if (str != NULL) {
      return str;
    }
  }
...
status_t Parcel::read(FlattenableHelperInterface &val) const {
  ...
  for (size_t i = 0; i < fd_count && err == NO_ERROR; i++) {
    ...
    if (fds[i] < 0) {
      err = BAD_VALUE;
      ALOGE(
          "dup() failed in Parcel::read, i is %zu, fds[i] is %d, "
          "fd_count is "
          "%zu, error: %s",
...
const flat_binder_object *Parcel::readObject(bool nullMetaData) const {
  ...
  if ((DPOS + sizeof(flat_binder_object)) <= mDataSize) {
    ...
    if (N > 0) {
      ALOGV("Parcel %p looking for obj at %zu, hint=%zu", this, DPOS, opos);
      ...
      if (OBJS[opos] == DPOS) {
        // Found it!
        ALOGV("Parcel %p found obj %zu at index %zu with forward search", this,
              DPOS, opos);
      ...
      if (OBJS[opos] == DPOS) {
        // Found it!
        ALOGV("Parcel %p found obj %zu at index %zu with backward search", this,
              DPOS, opos);
        mNextObjectHint = opos + 1;
        ALOGV("readObject Setting data pos of %p to %zu", this, mDataPos);
        return obj;
    ...
    ALOGW(
        "Attempt to read object from Parcel %p at offset %zu that is not "
        "in "
        "the object list",
        this, DPOS);
  }
...
uintptr_t Parcel::ipcData() const { return reinterpret_cast<uintptr_t>(mData); }
...
size_t Parcel::ipcObjectsCount() const { return mObjectsSize; }
...
void Parcel::ipcSetDataReference(const uint8_t *data, size_t dataSize,
  ...
  // ALOGI("setDataReference Setting data size of %p to %lu (pid=%d)", this,
  // mDataSize, getpid());
  mDataPos = 0;
  ...
  if (errorCheck() != NO_ERROR) {
    ...
    for (size_t i = 0; i < N; i++) {
      ...
      to << endl
         ...
         << TypeCode(flat->type & 0x7f7f7f00) << " = " << flat->binder;
    }
  } else {
    to << "NULL";
  }
...
status_t Parcel::restartWrite(size_t desired) {
  ...
  if (data) {
    LOG_ALLOC("Parcel %p: restart from %zu to %zu capacity", this,
...
status_t Parcel::continueWrite(size_t desired) {
  ...
  if (mOwner) {
    ...
    if (objectsSize < mObjectsSize) {
      ...
      for (size_t i = objectsSize; i < mObjectsSize; i++) {
        const flat_binder_object *flat =
            reinterpret_cast<flat_binder_object *>(mData + mObjects[i]);
        if (flat->type == BINDER_TYPE_FD) {
          // will need to rescan because we may have lopped off the
          // only FDs
          mFdsKnown = false;
        }
        release_object(proc, *flat, this, &mOpenAshmemSize);
    ...
    if (desired > mDataCapacity) {
      uint8_t *data = (uint8_t *)realloc(mData, desired);
      if (data) {
        LOG_ALLOC("Parcel %p: continue from %zu to %zu capacity", this,
                  mDataCapacity, desired);
        pthread_mutex_lock(&gParcelGlobalAllocSizeLock);
        gParcelGlobalAllocSize += desired;
        gParcelGlobalAllocSize -= mDataCapacity;
    ...
    if (!(mDataCapacity == 0 && mObjects == NULL && mObjectsCapacity == 0)) {
      ALOGE("continueWrite: %zu/%p/%zu/%zu", mDataCapacity, mObjects,
            mObjectsCapacity, desired);
    }
...
size_t Parcel::getBlobAshmemSize() const {
  // This used to return the size of all blobs that were written to ashmem,
  // now we're returning the ashmem currently referenced by this Parcel, which
  // should be equivalent.
  // TODO: Remove method once ABI can be changed.
  return mOpenAshmemSize;
}

size_t Parcel::getOpenAshmemSize() const { return mOpenAshmemSize; }

// --- Parcel::Blob ---

Parcel::Blob::Blob() : mFd(-1), mData(NULL), mSize(0), mMutable(false) {}
