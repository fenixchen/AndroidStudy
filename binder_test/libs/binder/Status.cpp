...
status_t Status::readFromParcel(const Parcel &parcel) {
  ...
  if (remote_stack_trace_header_size < 0 ||
      static_cast<size_t>(remote_stack_trace_header_size) >
          parcel.dataAvail()) {
    android_errorWriteLog(0x534e4554, "132650049");
    setFromStatusT(UNKNOWN_ERROR);
    return UNKNOWN_ERROR;
...
void Status::setServiceSpecificError(int32_t errorCode,
                                     const String8 &message) {
...
String8 Status::toString8() const {
  ...
  if (mException == EX_NONE) {
    ...
    ret.appendFormat("Status(%d, %s): '", mException,
                     exceptionToString(mException).c_str());
    if (mException == EX_SERVICE_SPECIFIC) {
