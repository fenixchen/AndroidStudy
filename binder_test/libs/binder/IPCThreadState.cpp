...
static const char *kReturnStrings[] = {"BR_ERROR",
                                       "BR_OK",
                                       "BR_TRANSACTION",
                                       "BR_REPLY",
                                       "BR_ACQUIRE_RESULT",
...
static const char *kCommandStrings[] = {"BC_TRANSACTION",
                                        "BC_REPLY",
                                        "BC_ACQUIRE_RESULT",
                                        "BC_FREE_BUFFER",
                                        "BC_INCREFS",
...
static const void *printBinderTransactionData(TextOutput &out,
                                              const void *data) {
  ...
  out << " (cookie " << btd->cookie << ")" << endl
      << "code=" << TypeCode(btd->code)
      << ", flags=" << (void *)(long)btd->flags << endl
      << "data=" << btd->data.ptr.buffer << " (" << (void *)btd->data_size
      << " bytes)" << endl
      << "offsets=" << btd->data.ptr.offsets << " ("
      << (void *)btd->offsets_size << " bytes)";
  return btd + 1;
}
...
sp<ProcessState> IPCThreadState::process() { return mProcess; }
...
pid_t IPCThreadState::getCallingPid() const { return mCallingPid; }
...
void IPCThreadState::blockUntilThreadAvailable() {
  ...
  while (mProcess->mExecutingThreadsCount >= mProcess->mMaxThreads) {
    ALOGW("Waiting for thread to be free. mExecutingThreadsCount=%lu "
          "mMaxThreads=%lu\n",
...
status_t IPCThreadState::transact(int32_t handle, uint32_t code,
  ...
  IF_LOG_TRANSACTIONS() {
    ...
    alog << "BC_TRANSACTION thr " << (void *)pthread_self() << " / hand "
         << handle << " / code " << TypeCode(code) << ": " << indent << data
         << dedent << endl;
  }
...
status_t IPCThreadState::attemptIncStrongHandle(int32_t handle) {
  ...
  printf("IPCThreadState::attemptIncStrongHandle(%ld) = %s\n", handle,
         result == NO_ERROR ? "SUCCESS" : "FAILURE");
#endif
...
IPCThreadState::IPCThreadState()
    : mProcess(ProcessState::self()), mMyThreadId(gettid()),
...
status_t IPCThreadState::waitForResponse(Parcel *reply,
  ...
  while (1) {
    ...
    IF_LOG_COMMANDS() {
      alog << "Processing waitForResponse Command: " << getReturnString(cmd)
           << endl;
    }
...
status_t IPCThreadState::talkWithDriver(bool doReceive) {
  ...
  IF_LOG_COMMANDS() {
    alog << "Our err: " << (void *)(intptr_t)err
         << ", write consumed: " << bwr.write_consumed << " (of "
         << mOut.dataSize() << "), read consumed: " << bwr.read_consumed
         << endl;
  }
...
status_t IPCThreadState::executeCommand(int32_t cmd) {
  ...
  case BR_ATTEMPT_ACQUIRE:
...

    {
      ...
      ALOG_ASSERT(success && refs->refBase() == obj,
                  "BR_ATTEMPT_ACQUIRE: object %p does not match cookie %p "
                  "(expected %p)",
                  refs, obj, refs->refBase());
  ...
  case BR_TRANSACTION: {
    ...
    IF_LOG_TRANSACTIONS() {
      TextOutput::Bundle _b(alog);
      alog << "BR_TRANSACTION thr " << (void *)pthread_self() << " / obj "
           << tr.target.ptr << " / code " << TypeCode(tr.code) << ": " << indent
           << buffer << dedent << endl
           << "Data addr = "
           << reinterpret_cast<const uint8_t *>(tr.data.ptr.buffer)
           << ", offsets addr="
    ...
    if (tr.target.ptr) {
      // We only have a weak reference on the target object, so we must first
      // try to safely acquire a strong reference before doing anything else
      // with it.
...
void IPCThreadState::freeBuffer(Parcel *parcel, const uint8_t *data,
  ...
  IF_LOG_COMMANDS() { alog << "Writing BC_FREE_BUFFER for " << data << endl; }
