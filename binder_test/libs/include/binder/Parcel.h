...
class Parcel {
 ...
 public:
  ...
  // immutable blobs if possible since they may be subsequently transferred
  // between processes without further copying whereas mutable blobs always need
  // to be copied. The caller should call release() on the blob after writing
  // its contents.
 ...
 private:
  ...
  class FlattenableHelperInterface {
   ...
   public:
    ...
    virtual status_t unflatten(void const *buffer, size_t size, int const *fds,
                               size_t count) = 0;
  };
  ...
  class FlattenableHelper : public FlattenableHelperInterface {
   ...
   public:
    virtual size_t getFlattenedSize() const { return val.getFlattenedSize(); }
    virtual size_t getFdCount() const { return val.getFdCount(); }
    virtual status_t flatten(void *buffer, size_t size, int *fds,
                             size_t count) const {
    ...
    virtual status_t unflatten(void const *buffer, size_t size, int const *fds,
                               size_t count) {
      return const_cast<Flattenable<T> &>(val).unflatten(buffer, size, fds,
                                                         count);
    }
  };
...
status_t Parcel::read(LightFlattenable<T> &val) const {
  ...
  if (size) {
    ...
    return buffer == NULL ? NO_MEMORY : val.unflatten(buffer, size);
  }
  return NO_ERROR;
}
...
void flatten_binder(const sp<ProcessState> &proc, const sp<IBinder> &binder,
                    flat_binder_object *out);
void flatten_binder(const sp<ProcessState> &proc, const wp<IBinder> &binder,
                    flat_binder_object *out);
