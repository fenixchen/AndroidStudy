...
status_t BnTestInterface::onTransact(uint32_t code, const Parcel &data,
                                     Parcel *reply, uint32_t flags) {
  switch (code) {
  case SAYHELLO: {
    CHECK_INTERFACE(ITestInterface, data, reply);
    sp<ICallbackInterface> callback =
        interface_cast<ICallbackInterface>(data.readStrongBinder());
